...
The BabyLM pipeline loads your model from the HuggingFace Hub and expects:
  * the FINAL model on the `main` branch
  * each training checkpoint on its OWN branch, named exactly like the local dir
    (chck_1M, chck_2M, ..., chck_10M, ...) so the `--fast` checkpoint eval can find them.
...
def main():
    ...
    api.upload_folder(folder_path=final, repo_id=args.repo_id, commit_message="final model")

    # 2) each checkpoint -> its own branch
    ckpt_root = os.path.join(args.out, "checkpoints")
    for name in sorted(os.listdir(ckpt_root), key=lambda s: int(s.split("_")[1].rstrip("M"))):
